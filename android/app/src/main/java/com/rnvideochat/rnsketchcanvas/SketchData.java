...
public class SketchData {
    ...
    public Rect addPoint(PointF p) {
        ...
        if (this.isTranslucent) {
            if (pointsCount >= 3) {
                addPointToPath(mPath, this.points.get(pointsCount - 3), this.points.get(pointsCount - 2), p);
            ...
            if (mDirty == null) {
                ...
                updateRect = new RectF(x - this.strokeWidth, y - this.strokeWidth, x + this.strokeWidth,
                        y + this.strokeWidth);
            } else {
                mDirty.union(x, y);
                updateRect = new RectF(mDirty.left - this.strokeWidth, mDirty.top - this.strokeWidth,
    ...
    private void draw(Canvas canvas, int pointIndex) {
        ...
        if (pointsCount >= 3 && pointIndex >= 2) {
            ...
            // This is so the next draw which uses a curve looks correct and continues from
            // there
