...
class SketchCanvas extends React.Component {
    ...
    constructor(props) {
        ...
        this.state.text = _processText(props.text ? props.text.map(t => Object.assign({}, t)) : null);
    }

    static getDerivedStateFromProps(props, state) {
        return {
            text: _processText(props.text ? props.text.map(t => Object.assign({}, t)) : null),
        };
    }
    ...
    addPath(data) {
        if (this._initialized) {
            ...
            const pathData = data.path.data.map(p => {
                ...
                return `${(coor[0] * this._screenScale * this._size.width) / data.size.width},${(coor[1] * this._screenScale * this._size.height) /
                    data.size.height}`;
            });
    ...
    save(imageType, transparent, folder, filename, includeImage, includeText, cropToImageSize) {
        UIManager.dispatchViewManagerCommand(this._handle, UIManager.getViewManagerConfig('RNSketchCanvas').Commands.save, [
            imageType,
            folder,
            filename,
            transparent,
    ...
    setPanResponder() {
        this.panResponder = PanResponder.create({
            ...
            onPanResponderGrant: (evt, gestureState) => {
                ...
                UIManager.dispatchViewManagerCommand(this._handle, UIManager.getViewManagerConfig('RNSketchCanvas').Commands.newPath, [
                    this._path.id,
        ...
        this.setState({ panResponderInitialized: true });
    }

    async componentDidMount() {
        this.setPanResponder();
        const isStoragePermissionAuthorized = await requestPermissions(this.props.permissionDialogTitle, this.props.permissionDialogMessage);
    }
